...
impl<const L: usize> BitOrAssign for BitArray<L> {
    fn bitor_assign(&mut self, rhs: Self) {
        for (a, b) in self.lanes.iter_mut().zip(rhs.lanes) {
            *a |= b;
        }
    }
}
    ...
    fn shl_assign(&mut self, n: usize) {
        ...
        if bits > 0 {
            for i in (0..L).rev() {
                self.lanes[i] <<= bits;
                if i > 0 {
                    self.lanes[i] |= self.lanes[i - 1] >> (64 - bits);
                }
            }
        }
    }
...
impl<const L: usize> Shr<usize> for BitArray<L> {
    type Output = Self;
    fn shr(mut self, n: usize) -> Self {
        self >>= n;
        self
    }
}

impl<const L: usize> super::SimHashBits for BitArray<L> {
    fn count_ones(self) -> usize {
        self.lanes.iter().map(|x| x.count_ones() as usize).sum()
    }
    fn to_u32_high_bits(self) -> u32 {
        (self.lanes[L - 1] >> 32) as u32
    }
